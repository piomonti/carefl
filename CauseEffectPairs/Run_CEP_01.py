...
# load flows
from nflib.flows import AffineConstantFlow, AffineFullFlow, MAF, NormalizingFlowModel, Invertible1x1Conv, ActNorm, AffineHalfFlow
...
def runCEPair( pair_id, Nlayers, Nhidden, priorDist='laplace', TrainSplit=1., epochs=100, optMethod='adam', removeOutliers=False, scaleDat=True, verbose=False ):
    """
    ...
    for l in Nlayers:
        for nh in Nhidden:
            ...
            results.loc[ (results.L==l) & (results.nh==nh), 'y->x' ] = np.nanmean( flow_mod_cond_rev.EvalLL( testDat_id[:,[1,0]] ) )
    ...
    return results, predModel, dir_id, np.minimum( np.unique(dat_id[:,0]).shape[0] / float(dat_id.shape[0]) , np.unique(dat_id[:,1]).shape[0] / float(dat_id.shape[0])  )
