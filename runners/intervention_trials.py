...
plt.plot(xvals,
         np.array([mod.predictIntervention(x0val=x, nSamples=10, dataDim=4, interventionIndex=1)[0, 2] for x in xvals]),
         linewidth=3, linestyle='-.', label=r'$X_2| do(X_1=x)$')
# and distribution of X_3 | do(X_0=x) should be quadratic
plt.plot(xvals,
         np.array([mod.predictIntervention(x0val=x, nSamples=10, dataDim=4, interventionIndex=1)[0, 3] for x in xvals]),
