...
from nflib.flows import AffineFullFlow, AffineFullFlowGeneral
...
class BivariateFlowCD:
    ...
    def __init__(self, Nlayers, Nhidden, priorDist='laplace', TrainSplit=1., epochs=100, optMethod='adam',
                 verbose=False):
        ...
        for l in self.Nlayers:
            for nh in self.Nhidden:
                ...
                if self.priorDist == 'laplace':
                    ...
                    # TransformedDistribution(Laplace(torch.zeros( Ncomp ), torch.ones( Ncomp )),
                    #                                                  SigmoidTransform().inv)
                    # MultivariateNormal(loc=np.zeros((Ncomp,)), covariance_matrix = np.eye( Ncomp )).inv )
                    #                                                       SigmoidTransform().inv)
                else:
                ...
                # now we train this model and store the likelihood:
                loss_cond_rev = flow_mod_cond_rev.train(epochs=self.epochs, optMethod=self.optMethod,
                ...
                results.loc[(results.L == l) & (results.nh == nh), 'y->x'] = np.nanmean(
                    flow_mod_cond_rev.EvalLL(testDat[:, [1, 0]]))
